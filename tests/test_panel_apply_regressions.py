...
REG-22 (LED-10): на непоказанную карточку НЕ повторяется ничего — ни команда
    применения, ни запись. Повтор возвращали сюда дважды, и оба раза он делал
    хуже; второй раз это стоило трети всех показов и полугода репутации табло.
    A/B на живой панели 23.09.2026 (10.30.205.76, блоками по 14, по 28
    карточек в плече): с повтором 17/28 = 60% и 36% у следующей карточки после
    провала; без повтора 25/28 = 89% и 100% у следующей. Плечо «с повтором»
    совпало с боевой долей за 48 ч (61%, n=257) — опыт воспроизводит прод, а не
    лабораторию. Механизм: панель платит за СЕАНС, а не за карточку; провал уже
    стоит двух сеансов, повтор добавляет ещё два, и топят они СЛЕДУЮЩУЮ
    карточку. Отложенный повтор тоже не выход: карта не помнит записанную
...
def test_reg22_failed_show_is_not_retried_at_all(panel, monkeypatch):
    ...
    Плечо «с повтором» совпало с боевой долей за 48 ч (61%, n=257) - опыт
    воспроизводит прод.

    Механизм: цена для панели - СЕАНС, а не карточка. Провал уже стоит двух
    сеансов, повтор добавляет ещё два, и они топят следующую карточку.
    ...
    A/B и посмотрите на СЛЕДУЮЩУЮ карточку, а не на текущую.
    """
    applies = {"n": 0}
    file_frames = []
