...
#: Сколько файлов ОБЯЗАНО быть в активной программе: кадр и boot-файл.
#:
#: Это не догадка о формате, а измеренное поведение исправной панели. Замер
#: 23.09.2026 на 10.30.205.75: до карточки список 0x0012 был
#: ['2e4c0b31', '97668a76'], после - ['0aecf8ad', '806e601e']. Список
#: ЗАМЕНИЛСЯ целиком и остался длиной два. Значит панель программу заменяет, а
#: не дополняет.
#:
#: Ровно в тот же день вторая панель объекта (10.30.205.76) показала, что
#: бывает, когда замена ломается: список разросся до 231 записи, все разные, и
#: карта перестала грузить программу вовсе - `ProgramIndex index="-1"`,
#: `ProgramCount count="0"`, имя программы пустое. Экран почернел.
#:
#: Самое дорогое в том отказе - не он сам, а МОЛЧАНИЕ. Проверка показа тогда
#: сводилась к «наш md5 есть в списке», а при 231 записи это выполняется почти
#: всегда: сервис час подряд рапортовал `ok=True` на чёрный экран. Длина
#: списка - самый ранний и самый дешёвый признак этой беды, поэтому она
#: проверяется на каждой карточке и уходит наружу.
EXPECTED_PROGRAM_FILES = 2

#: С какой длины списка панель считается сломанной и в неё перестают писать.
#:
#: Порог выше двух намеренно: одиночный лишний файл может быть гонкой между
#: нашей заливкой и опросом, и гасить из-за неё рабочее табло нельзя. А вот
#: устойчивый рост - это уже начавшееся слипание программы, и каждая
#: следующая карточка делает его хуже, потому что удалить записи нечем:
#: в протоколе 9527 команды удаления нет, а `DeleteFiles` старого SDK на порту
#: 10001 отвечает пустым `<out>` и не выполняется (проверено, LED-INTEGRATION §10).
#:
#: То есть «почистить потом» не получится. Единственное, что мы можем, -
#: вовремя перестать лить.
PROGRAM_FILES_BROKEN_AT = 8
...
class SendResult:
    ...
    detail: str = ""
    #: Длина списка активной программы на момент проверки. Наружу уходит
    #: всегда: по ней видно слипание программы ЗАДОЛГО до того, как экран
    #: погаснет, и без неё отказ снова будет молчаливым.
    program_files: int = 0
    #: Панель перестала заменять программу и копит ссылки. Писать в неё
    #: дальше нельзя - чистить нечем.
    degraded: bool = False
...
class Panel:
    ...
    def program_health(self) -> tuple[int, bool]:
        """
        Сколько файлов в активной программе и не начала ли она слипаться.

        Возвращает (сколько, сломана ли). Отдельным методом, а не проверкой по
        месту, потому что спрашивают её два разных потребителя: отправка
        карточки и опрос состояния для мониторинга, и разъехаться им нельзя.

        Недоступность панели тут НЕ считается поломкой программы: это другая
        беда, у неё свои сообщения, и путать их - значит гасить табло из-за
        одного пропавшего пакета.
        """
        try:
            n = len(self.files_on_panel())
        except OSError:
            return 0, False
        return n, n >= PROGRAM_FILES_BROKEN_AT

    def shown_now(self, md5: str) -> tuple[bool, str]:
        """
        Показывает ли панель ИМЕННО наш кадр. Возвращает (да/нет, чем измерено).

        Два измерителя, и порядок между ними не вкусовой.

        0x0013 -> 0x0014 отвечает на нужный вопрос - «что на экране сейчас».
        В коде он был объявлен мёртвым: «на панелях объекта не отвечает вовсе,
        проверено на обеих двумя клиентами» (21.09.2026). Замер 23.09.2026
        показал, что это верно НЕ ВЕЗДЕ: на 10.30.205.75 он молчит по-прежнему,
        а на 10.30.205.76 отвечает устойчиво - шесть опросов из шести, один и
        тот же хеш, и он есть в списке файлов. Поэтому спрашиваем его первым, а
        «не отвечает» трактуем как «этого измерителя здесь нет», а не как отказ
        показа: именно эта трактовка когда-то объявляла неудачей каждый показ.

        0x0011 -> 0x0012 (список файлов) - ЗАПАСНОЙ и заведомо слабый. Про него
        в REG-21 сказано, что он отдаёт состав активной программы, «после
        нескольких сотен карточек по-прежнему две записи». Замер 23.09.2026
        это опроверг: на 10.30.205.76 в ответе 89 записей, на 10.30.205.75 - две.
        Девяносто записей означают, что «наш md5 есть в списке» выполняется
        почти всегда сразу после записи, то есть проверка перестаёт что-либо
        проверять. Видно и снаружи: 12:09-12:41 сервис отдал ok=True на 20
        карточках подряд по этой панели, чего не бывает.

        Поэтому счёт файлов возвращается наружу: в `detail` его видно, и
        «проверено списком из 89 файлов» читается как предупреждение, а не как
        подтверждение. Молчаливая деградация измерителя - худшее, что тут может
        быть: она маскирует погасшее табло.
        """
        try:
            active = self.active_program_md5()
        except OSError as e:
            return False, f"0x0014 недоступен: {e}"
        if active is not None:
            return active == md5, "0x0014 (что на экране сейчас)"

        files = self.files_on_panel()
        weak = " — измеритель ненадёжен, список слишком длинный" if len(files) > 8 else ""
        return md5 in files, f"0x0012, файлов в ответе {len(files)}{weak}"
    ...
    def send_png(self, img: bytes, width: int | None = None, height: int | None = None,
        ...
        """
        # СНАЧАЛА спрашиваем, в каком состоянии программа на карте, и только
        # потом пишем. Порядок именно такой, потому что запись необратима:
        # удалить файл из программы нечем ни в протоколе 9527, ни через старый
        # SDK. Каждая карточка, записанная в уже слипшуюся программу, делает её
        # длиннее и отдаляет починку.
        #
        # Так была потеряна панель 10.30.205.76: список рос с каждой машиной,
        # пока карта не перестала грузить программу вовсе. Проверка стоила бы
        # одного опроса.
        n_files, broken = self.program_health()
        if broken:
            return SendResult(
                False, False, "", files_on_panel=[], program_files=n_files, degraded=True,
                detail=(
        ...
        # ОДИН опрос состава активной программы. Ни трёх попыток, ни повторного
        # применения - и то и другое здесь вредило.
        #
        # Почему не `active_program_md5()` (кадр 0x0013 -> 0x0014), как было
        # раньше: на панелях объекта этот запрос НЕ ОТВЕЧАЕТ. Замерено
        # 21.09.2026 на обеих (10.30.205.75 и .76), двумя независимыми путями -
        # и самим сервисом, и отдельным клиентом: ответ 0x0014 не приходит,
        # `active_program_md5()` стабильно возвращает None. Проверка не
        # сходилась НИКОГДА, а значит: любой показ объявлялся неудачей, и после
        # каждого несовпадения повторялась команда применения. На живом потоке
        # машин это давало три переключения программы на одну карточку -
        # непрерывное моргание табло, которое и заметил заказчик.
        #
        # ⚠️ Здесь было сказано, что 0x0012 отдаёт СОСТАВ АКТИВНОЙ ПРОГРАММЫ и
        # что это видно по количеству - «после нескольких сотен показанных
        # карточек список по-прежнему из двух записей». Замер 23.09.2026 это
        # опроверг: на 10.30.205.76 в ответе 89 записей (на .75 - две). Список
        # из 89 делает проверку «наш md5 есть в ответе» истинной почти всегда
        # сразу после записи, то есть измеритель молча перестаёт мерить.
        #
        # Выбор измерителя и его слабые места - в `shown_now`. Сюда важно
        # донести одно: чем проверяли, попадает в `detail` и уходит наверх.
        time.sleep(APPLY_WAIT)
        try:
            files = self.files_on_panel()
            ok_shown, how = self.shown_now(md5)
        except OSError as e:
            return SendResult(
                True, False, md5,
        ...
        if ok_shown:
            return SendResult(True, True, md5, files_on_panel=files,
                              program_files=len(files),
                              degraded=len(files) > EXPECTED_PROGRAM_FILES,
                              detail=f"показывается ({slot}); проверено: {how}")

        # РОВНО ОДНА повторная команда показа - и только теперь, когда точно
        # известно, что первая не сработала.
        #
        # Панель роняет программы случайно: замер 21.09.2026, десять карточек
        # подряд на .76 дали «нет, нет, нет, нет, нет, нет, да, да, да, нет» -
        # ни от пауз между карточками, ни от их числа узор не зависит. При
        # такой доле отказов вторая попытка окупается.
        #
        # Повторяется именно ПРИМЕНЕНИЕ, а не запись: файл уже лежит на карте
        # целым, и переписывать его нельзя - новое имя добавило бы на карту
        # ещё один неудаляемый файл, а именно их накопление панель и убивает.
        #
        # Попытка одна, а не три, как было раньше. Тогда повторы шли вслепую
        # (проверка не работала в принципе) и моргали табло на КАЖДОЙ карточке.
        # Теперь моргнуть может только та, что действительно не встала.
        try:
            self._apply_program()
        except OSError as e:
            return SendResult(
                True, False, md5, files_on_panel=files,
                detail=f"кадр записан в {slot}, повторное применение не прошло: {e}",
            )
        ...
        try:
            ...
            return SendResult(
                True, False, md5,
                detail=f"кадр записан в {slot}, проверка после повтора не удалась: {e}",
            )

        if ok_shown:
            return SendResult(
                True, True, md5, files_on_panel=files,
                program_files=len(files),
                degraded=len(files) > EXPECTED_PROGRAM_FILES,
                detail=f"показывается со второй команды ({slot}); проверено: {how}",
            )

        return SendResult(
            False, False, md5, files_on_panel=files,
            program_files=len(files),
            degraded=len(files) > EXPECTED_PROGRAM_FILES,
            detail=(
                f"кадр записан в {slot}, но в активной программе его нет даже после "
                f"повтора - панель проглотила карточку. В активной программе "
                f"{len(files)} файлов при норме {EXPECTED_PROGRAM_FILES}."
            ),
        )
