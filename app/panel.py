...
class Panel:
    ...
    def send_png(self, img: bytes, width: int | None = None, height: int | None = None,
        ...
        три раза вслепую, потом один раз по факту отказа, - и оба раза он
        делал хуже. Обоснование и числа A/B - ниже, у самого места отказа;
        коротко: панель платит за СЕАНС, лишние сеансы топят следующую
        карточку, и «повторить бы ещё разок» здесь стоит трети всех показов.
        ...
        # ПОВТОРА НЕТ, И ЭТО ГЛАВНОЕ РЕШЕНИЕ ЭТОЙ ФУНКЦИИ. Не «упрощение» -
        # повтор измеренно ВРЕДИЛ, и вернуть его значит вернуть треть потерь.
        #
        ...
        # вторая попытка казалась выгодной. Замер смотрел только на ТЕКУЩУЮ
        # карточку и не мог увидеть, что повтор делает со следующей.
        #
        # A/B на живой панели 23.09.2026, блоками по 14, два плеча по 28
        # карточек на 10.30.205.76:
        #   * с повтором  - встало 17/28 = 60%, после провала следующая 36%;
        #   * без повтора - встало 25/28 = 89%, после провала следующая 100%.
        # Плечо «с повтором» совпало с боевой долей за 48 ч (61%, n=257) -
        # значит опыт воспроизводит прод, а не лабораторные условия.
        #
        # Механизм: панель платит не за карточку, а за СЕАНС. Провал и так
        # стоит двух сеансов (запись и сверка), повтор добавляет ещё два -
        # применение и вторую сверку. Именно эти два лишних сеанса топят
        # СЛЕДУЮЩУЮ карточку: в боевых логах после провала встаёт 31%, после
        # успеха 79%. Без повтора «прилипчивость» отказа исчезает вовсе.
        #
        ...
        #
        # Непоказанную карточку повторяет вызывающий - следующим проездом или
        # пересчётом оператора, - и к тому моменту панель успевает отдохнуть.
        return SendResult(
            False, False, md5, files_on_panel=files,
            detail=(
                f"кадр записан в {slot}, но в активной программе его нет - панель "
                f"проглотила карточку. Повтор здесь НЕ делается намеренно: замерено, "
                f"что он роняет показ следующей карточки (36% против 100%)."
            ),
        )
