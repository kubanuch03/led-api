...
class Panel:
    ...
    def send_png(self, img: bytes, width: int | None = None, height: int | None = None,
        ...
        ОДНА ПОПЫТКА, БЕЗ ПОВТОРОВ. Сюда дважды возвращали повтор - сначала
        три раза вслепую, потом один раз по факту отказа. Замер 23.09.2026
        говорит, что и второй вариант делал хуже: рабочая гипотеза - панель
        платит за СЕАНС, и лишние сеансы топят следующую карточку. Числа A/B
        и, главное, чем этот замер испорчен - ниже, у самого места отказа.
        Читать их обязательно, прежде чем возвращать повтор в третий раз.
        ...
        #
        # Кадр 0x0011 -> 0x0012 на тех же панелях работает и отвечает СОСТАВОМ
        # АКТИВНОЙ ПРОГРАММЫ - двумя именами, кадром и boot-файлом. Что это
        # именно активная программа, а не «все файлы карты», видно по
        # количеству: после нескольких сотен показанных карточек список
        # по-прежнему состоит из двух записей. Поэтому «наш md5 есть в активной
        # программе» - честный ответ на вопрос «видит ли это водитель», а не
        # ослабление проверки.
        ...
        if md5 in files:
            return SendResult(True, True, md5, files_on_panel=files, detail=f"показывается ({slot})")

        # ПОВТОРА НЕТ. Решение принято по замеру, но замер НЕ ЧИСТЫЙ, и это
        # честно сказано ниже - чтобы следующий человек знал, что именно
        # перепроверять, а не наткнулся на уверенный тон и поверил на слово.
        #
        # Здесь стояла одна повторная команда показа. Её оправдывал замер
        # 21.09.2026: десять карточек подряд, шесть отказов - при такой доле
        # вторая попытка казалась выгодной. Тот замер смотрел только на
        # ТЕКУЩУЮ карточку и не мог увидеть, что повтор делает со следующей.
        #
        # РАБОЧАЯ ГИПОТЕЗА (не установленный механизм): панель платит не за
        # карточку, а за СЕАНС. Провал и так стоит двух сеансов - запись и
        # сверка, - а повтор добавляет ещё два: применение и вторую сверку.
        # Эти лишние сеансы и топят СЛЕДУЮЩУЮ карточку.
        #
        # ЧТО ЗАМЕРЕНО. A/B на живой панели 23.09.2026, 10.30.205.76, блоками
        # по 14, два плеча по 28 карточек:
        #   * с повтором  - встало 17/28 = 60%, после провала следующая 36%;
        #   * без повтора - встало 25/28 = 89%, после провала следующая 100%.
        # Точный критерий Фишера по общей разнице: p = 0.029, 95% ДИ разницы
        # +5.8...+48.2 п.п. Разница «после провала» (4/11 против 3/3) не
        # значима вовсе: p = 0.19.
        #
        # ЧЕМ ЗАМЕР ИСПОРЧЕН, И ЭТО ВАЖНЕЕ ЧИСЕЛ. Опыт шёл ОТДЕЛЬНЫМ процессом
        # со своим экземпляром `Panel`, то есть без `_busy` из `main.py` и без
        # общего `_lock`. За те же 24 минуты сервис отправил на эту панель 15
        # боевых карточек. Чужая карточка, попавшая между нашей записью и
        # нашей сверкой, читается как отказ - и наоборот: с середины опыта
        # боевые карточки массово пошли с `verified=False`, то есть опыт мешал
        # им ровно так же. Направление эффекта правдоподобно, величина -
        # нет.
        #
        # ЧЕМ ПРОВЕРИТЬ ЧИСТО: выкатить эту версию на ОДНУ панель и оставить
        # вторую контролем, затем сравнить боевые доли за несколько часов по
        # логам `tasks.py:945`. Никакого постороннего трафика, настоящий путь
        # кода, одновременный контроль. Пока этого не сделано, «89%» - оценка,
        # а не факт.
        #
        # Отдельно: боевая «прилипчивость» отказа (после успеха 79%, после
        # провала 31%, n=257) гипотезу НЕ подтверждает. В проде повтор
        # назначается именно после отказа, поэтому те же числа одинаково
        # хорошо объясняются тем, что панель просто уходит в полосы отказов.
        #
        # Почему нельзя «повторить чуть позже, но с перезаписью»: карта не
        # помнит записанную программу. Проверено 23.09.2026 выдержкой 45 с -
        # `_apply_program()` без новой записи кадр не поднимает (три раунда из
        # трёх). То есть отложенный повтор стоил бы нового неудаляемого файла
        # на карте, а не одной дешёвой команды.
        #
        # И чего здесь НЕ надо додумывать: непоказанную карточку не «повторит
        # следующий проезд» - следующей машине нужна ДРУГАЯ карточка. Эта
        # конкретная сумма этому конкретному водителю показана не будет.
        # Вернуть её может только оператор пересчётом.
        return SendResult(
            False, False, md5, files_on_panel=files,
            detail=(
                f"кадр записан в {slot}, но в активной программе его нет - панель "
                f"проглотила карточку. Повтор здесь НЕ делается намеренно: по замеру "
                f"он ухудшает показ следующей карточки."
            ),
        )
